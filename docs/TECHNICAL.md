...
1. 后台 → **推广员** → 新增（推广码、名称、佣金比例%，默认 10%）
2. 把专属链接发给合作方，例如：`https://你的域名/zh?ref=ZHANGSAN`
3. 访客点击后 Cookie 记住推广码 **30 天**；期间下单并**付款成功** → 生成提成（状态：待结算）
4. 计佣基数 = **商品小计 − 优惠**（不含运费、税）
5. 后台将订单标为 **已完成** → 提成变为 **可结算**；全额退款 / 取消 → **作废**
6. 后台 → **推广提成** → 人工打款后点「标记已打款」

> 当前为链接归因，不依赖优惠券。优惠券仍可单独给买家打折，与推广归因可同时存在。
