...
2. 后台 → **推广员** → 新增 → **搜索并选择该用户** → 填推广码与佣金比例
3. 对方用 **前台登录**（非后台）→ **我的账户 → 我的推广**，可查看：推广状态、专属链接、提成金额与结算状态（待结算 / 可结算 / 已打款 / 已作废）
4. 一个前台用户只能绑定一个推广员档案；未绑定用户无法打开前台推广看板（会提示联系商家开通）

#### 链接推广

1. 后台创建推广员并绑定用户后，把专属链接发给合作方，例如：`https://你的域名/zh?ref=ZHANGSAN`
