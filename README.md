...
- **链接推广提成**（绑定前台用户；`?ref=` 专属链接，付款后计佣；用户登录「我的推广」可查结算）
- **优惠券推广**（优惠码可绑定推广员，用券下单计佣；优先于链接 Cookie）
...
| **推广员 / 提成** | 后台 `/admin/affiliates`、`/admin/commissions`；前台 `/account/affiliate`（普通用户登录查状态） |
